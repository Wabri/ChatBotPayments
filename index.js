...
class Settings {
    constructor(version, sessionID, expressPort, rasaIP, rasaPort, token) {
...
const settingsApp = new Settings(process.env.VERSION_APP, process.env.SESSION_ID, process.env.SERVER_PORT_EXPRESS, process.env.SERVER_IP_RASA, process.env.SERVER_PORT_RASA, process.env.TOKEN_RASA);
...
socketIOServer.on("connection", socket => {
    socket.on("userMessage", (messageReceive) => {
        ...
        const rasaAddress = "http://" + settingsApp.rasaIP + ":" + settingsApp.rasaPort;
        const userID = socket.id;
        if (messageReceive != "conversation reset default") {
            /* esecuzione della post al server di rasa che eseguirà il parse del
           messaggio dell'utente*/
            comunicationRasaManager.questionAndAnswer(socket, messageReceive, rasaAddress, userID, settingsApp.token);
        }
        else {
